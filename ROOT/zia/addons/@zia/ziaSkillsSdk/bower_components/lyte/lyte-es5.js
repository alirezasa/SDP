...
if (document.readyState === "complete" || document.readyState === "interactive") {
	...
	function Router(routes) {
		...
		var functionStr = "function",
		    ...
		    beforeExitWarning = beforeExitStr + ' callback is deprecated and will be removed in next version. Move code from ' + beforeExitStr + ' to ' + didDestroyStr + ' or ' + willTransitionStr + ' method. \n\tUsage : ',
		...
		run[beforeExitStr] = function (hook, index) {
			...
			if (callback) {
				Lyte.warn(beforeExitWarning + prevTransRouteInstance.__lp.objPath + " " + routeStr);
			}
   ...
   */
		...
		this.configureRoutes = function (map) {
			...
			config.routes = (typeof map === 'undefined' ? 'undefined' : _typeof(map)) == functionStr ? parseRouteMapping.call(this, map) : map;
			...
			var LinkTo = function (_HTMLElement2) {
				...
				_createClass(LinkTo, [{
					...
					value: function setCustomAttributes(linkTag, onlyLtProp) {
						for (var _key in this.ltProp) {
							if (/^(id|class|style|target)$/.test(_key)) {
								linkTag.setAttribute(_key, this.ltProp[_key]);
							}
						}
						if (!onlyLtProp) {
							for (var _i2 = 0, attr, attrName; attr = this.attributes[_i2]; _i2++) {
								if ((attrName = attr.nodeName) !== "lt-prop" && /^(lt-prop-id|lt-prop-class|lt-prop-style|lt-prop-target)$/.test(attrName)) {
		...
		function consoleErrorFromCallback(err, hook, routeName, state) {
			if (!Array.isArray(err) && !err.$) {
				...
				consoleError(state, hook, routeName);
			}
		}
		...
		function processError(stopTrans, err, instance, options) {
			...
			if (trans.prom) {
				...
				if (!stopTrans) {
					...
					trans.pause();
					run[onErrorStr].call(this, hook, index, err);
				}
			}
		}
		...
		function setMixinPropInDef(mixin, fns) {
			for (var key in mixin) {
				if (key == "actions") {
					Object.assign(fns.actions || {}, mixin.actions);
				} else {
					fns[key] = mixin[key];
				}
			}
		...
		function setMixinPropInDefandIns(mixin, fns, prev, curr) {
			for (var key in mixin) {
				if (key == "actions") {
					...
					prev.actions = curr.actions = Object.assign(fns.actions[action], mixin.actions[action]);
				} else {
 ...
 */
	...
	var Test = function (_HTMLElement3) {
		_inherits(Test, _HTMLElement3);

		function Test() {
			_classCallCheck(this, Test);

			var _this3 = _possibleConstructorReturn(this, (Test.__proto__ || Object.getPrototypeOf(Test)).call(this));

			if (!_this3.attributes.t) {
				_LC.frSpecial = true;
			}
			return _this3;
		}
	...
	var LyteYield = function (_HTMLElement4) {
		...
		_createClass(LyteYield, [{
			...
			value: function disconnectedCallback() {
				...
				for (var _key4 in this._dynamicProperty) {
					if (this._dynamicProperty[_key4].isActualNode) {
						this._dynamicProperty[_key4].isActualNode._helperNodes[del](this);
					} else {
						var helperNodes = node._callee.getProperty(_key4)._helperNodes;
						if (helperNodes) {
	...
	var customElementPrototype = function (_elementPrototype) {
		...
		function customElementPrototype() {
			...
			if (!_LyteComponent.registeredComponents[_this5.localName]) {
				return _possibleConstructorReturn(_this5);
			}
			_this5.constructor.activeInstances++;
			var fastRenderIndex = void 0;
			var lytePropAttr = _this5.attributes._lyteprop;
			...
			var compInstance = _this5.component = fastRenderedProp ? fastRenderedProp.component : new _this5.constructor.component();
			_this5._properties = {};
			...
			if (!fastRenderedProp) {
				...
				var data = _this5.constructor._data ? _this5.constructor._data() : {};
				var def = "default";
				for (var _key5 in data) {
					compData[_key5] = data[_key5][def];
				}
				compData.errors = {};
			...
			for (var _key6 in _this5.constructor._methods) {
				compInstance.methods[_key6] = _this5.constructor._methods[_key6];
			}
			if (compInstance.$node) {
				for (var key in compInstance.$node) {
					_this5[key] = compInstance.$node[key];
				}
			}
			compInstance.$node = _this5;
			if (!fastRenderedProp) {
				_this5.callback("constructor");
				_this5._actions = _this5._actions ? _this5._actions : {};
				_this5._callee = _this5._callee || _this5.getCallee(_this5.parentNode);
		...
		_createClass(customElementPrototype, [{
			...
			value: function afterConnected() {
				...
				for (var _i6 = 0; _i6 < obsattr.length; _i6++) {
					var _key7 = obsattr[_i6];
					var prop = this.getProperty(_key7);
				...
				if (this._initProperties) {
					...
					for (var _key8 in initProperties) {
						var actVal = void 0;
						if (compInstance.__data[_key8] && compInstance.__data[_key8].type !== _LC.getDataType(initProperties[_key8]) && (initProperties[_key8] !== undefined || compInstance.__data[_key8].type === "boolean")) {
							actVal = _LC.typeCast(initProperties[_key8], compInstance.__data[_key8].type);
						} else {
							actVal = initProperties[_key8];
						}
						var error = _LC.handleValidation(compData, _key8, actVal, compInstance);
						if (!error) {
							compData[_key8] = actVal;
						}
					}
				...
				if (this._initMethods) {
					...
					for (var _key9 in initMethods) {
						compInstance.methods[_key9] = initMethods[_key9];
					}
			...
			value: function renderComponent(dynNode, preDefObj) {
				...
				component.data = {};
				var data = constr._data ? constr._data() : {};
				var def = "default";
				for (var _key10 in data) {
					component.data[_key10] = data[_key10][def];
				}
				...
				if (initProperties) {
					for (var _key11 in initProperties) {
						var actVal = void 0;
						if (component.__data[_key11] && component.__data[_key11].type !== _LC.getDataType(initProperties[_key11]) && (initProperties[_key11] !== undefined || component.__data[_key11].type === "boolean")) {
							actVal = _LC.typeCast(initProperties[_key11], component.__data[_key11].type);
						} else {
							actVal = initProperties[_key11];
						}
						var error = _LC.handleValidation(component.data, _key11, actVal, component);
						if (!error) {
							component.data[_key11] = actVal;
						}
					}
				}
				if (true) {
								...
								case "forIn":
									{
										...
										if (object && (objKeys = Object.keys(object))) {
											...
											for (var keyIndex = 0; keyIndex < objKeys.length; keyIndex++) {
												var _key12 = objKeys[keyIndex];
												compData[valueKey] = object[_key12];
												compData[keyKey] = _key12;
								...
								case "attr":
									{
										...
										for (var k = inte + 1; k < dynLength; k++) {
											var locDyn = dynamicNodes[k];
										...
										for (var key in attr) {
											if (attr[key].dynamicValue) {
												...
												var nodeValue = attr[key].newDynamicValue ? _LC.getDD(compData, attr[key].newDynamicValue) : this.get(attr[key].dynamicValue);
												...
												if (attr[key].helperInfo.name === "action") {
													...
													var _nodeValue = this.processHelper({ name: attr[key].helperInfo.name, args: this.processArgs(this, attr[key], [], undefined, undefined, true) }, undefined);
													if (fastRenderProp) {
								...
								case "insertYield":
									{
										...
										if (yieldObj) {
											...
											for (var _key13 in contextSwitch) {
												parentScope.component.data[_key13] = contextSwitch[_key13];
											}
											if (yieldObj._cx) {
        ...
        content = toAppend.cloneNode(true, "lyte");
				...
				for (var _i8 = 0; _i8 < dynamicNodes.length; _i8++) {
					...
					if (type === "componentDynamic") {
						...
						this.bindNode(dynamicN, undefined, helperNode, options, dynamicNodes[_i8], processLast, establishBindings);
					} else if (type === "attr") {
						...
						var toBeRemoved = [];
						for (var _key14 in info.attr) {
							var _attr = info.attr[_key14];
          ...
          */
			...
			value: function attributeChangedCallback(attr, oldValue, newValue) {
				...
				if (isCustomProp !== -1) {
					...
					if (lyteProps) {
						try {
							...
							for (var _key15 in lyteProps) {
								var actKey = propValue + _LC.String.upperCaseFirstLetter(_key15);
								this.set(actKey, lyteProps[_key15]);
							}
						} catch (e) {
				...
				var compInstance = this.component;
				var dataType = compInstance.__data[actualAttr].type;
				if (dataType !== "string") {
			...
			value: function removeHelpers(node, update, direct) {
				...
				if (node._forContent) {
					if (node.getAttribute("is") === "for") {
						...
						var _key16 = node.getAttribute("item");
						if (node._items.length) {
							...
							if (prop) {
								for (var _i16 = 0; _i16 < node._items.length; _i16++) {
									this.removeBindings(_defineProperty({}, _key16, node._items[_i16].itemProperty), _defineProperty({}, _key16, node._attributes.items[_i16]));
								}
							}
						}
						if (!update) {
						...
						var _key17 = node.getAttribute("key");
						for (var index in items) {
							...
							if (_prop) {
								this.removeBindings(_defineProperty({}, _key17, node._items[index].itemProperty), _defineProperty({}, _key17, node._attributes.object[index]));
							}
						}
						if (!update) {
							if (node._actualBinding) {
					...
					if (node._caseContent) {
						...
						for (var _key18 in node._dynamicProperty) {
							if (node._dynamicProperty[_key18].isActualNode) {
								node._dynamicProperty[_key18].isActualNode._helperNodes[del](node);
							} else {
								var helperNodes = this.getProperty(_key18)._helperNodes;
								if (helperNodes) {
					...
					for (var _key19 in node._renderedComponent) {
						if (node._renderedComponent[_key19]) {
							node._renderedComponent[_key19].remove();
							node._renderedComponent[_key19] = null;
						}
					}
				}
            ...
            if(forItem._helperNodes){
				...
				if (options.type !== "remove") {
					...
					for (var _k3 = firstIndexForIteration, _v4 = lastIndexForIteration; _v4 > 0; _k3++, _v4--) {
						...
						for (var d = 0; d < dynProps.length; d++) {
							var _key21 = dynProps[d];
							node._dynProps[_key21] ? node._dynProps[_key21]++ : node._dynProps[_key21] = 1;
						}
						...
						if (!node._fRender) {
							...
							domArr.push(node._forContent[_k3] = []);
						}
				...
				_LC.unbound = initialUnbound;
				for (var _i29 = lastIndexForIteration; _i29 < node._items.length; _i29++) {
			...
			value: function updateNode(node, updatePath) {
				...
				if (node.helperValue) {
					...
					var value = path ? _LC.get(compInstance.data, path, dynamicValues) : compInstance.data[_boundValue2];
			...
			key: 'processArgs',
			value: function processArgs(scope, dynN, dynamicValues, event, node, newCompile) {
				...
				for (var _i36 = 0; _i36 < args.length; _i36++) {
					if (args[_i36] && args[_i36].type) {
						this.internalHelpers(scope, args, _i36, dynamicValues, event, node, newCompile);
					} else {
						if (!this.isEmptyString(args[_i36])) {
							if (args[_i36].startsWith("'") && args[_i36].endsWith("'")) {
								...
								if (args[_i36] === "event" && event) {
									...
									args[_i36] = _LC.get(scope.component.data, args[_i36], dynamicVals);
								}
			...
			key: 'internalHelpers',
			value: function internalHelpers(scope, args, i, dynamicValues, event, node, newCompile) {
				//helperFunc = this.constructor.getHelper(args[i]);
				var helperFunc = args[i].value;
				var helperVal = this.processHelper({ "name": helperFunc.name, "args": this.processArgs(scope, { "helperInfo": helperFunc }, dynamicValues, event, node, newCompile) });
				args[i] = helperVal;
			}
		}, {
			key: 'processHelper',
			...
			value: function registerYields() {
				...
				var _loop = function _loop(_i38) {
					var parentYield = void 0;
					if (yields[_i38].hasAttribute("from-parent") && _this7._callee && (parentYield = _this7._callee._yields[yields[_i38].getAttribute("yield-name")])) {
						var locYield = void 0;
						if (_Lyte._ie) {
							locYield = document.createElement("div");
							locYield.innerHTML = parentYield.outerHTML;
							locYield = locYield.childNodes[0];
							_this7.constructor.splitTextNodes(locYield);
						} else {
						...
						_this7._yields[yields[_i38].getAttribute("yield-name")] = locYield;
					} else {
						_this7._yields[yields[_i38].getAttribute("yield-name")] = yields[_i38];
					}
				};
			...
			value: function actualConnectedCallback() {
				...
				if (templateAttributes && templateAttributes.attr) {
					for (var _key22 in templateAttributes.attr) {
						//        		let attr = templateAttributes.attr[i];
						var attr = templateAttributes.attr[_key22];
						if (!this.hasAttribute(attr.name) && !compData.hasOwnProperty(attr.name) || attr.globalEvent) {
							...
							if (!attr.globalEvent) {
								attr.from = "component";
								if (attr.staticValue) {
									this.setAttribute(templateAttributes.attr[_key22].name, attr.staticValue);
								} else {
									this.setAttribute(templateAttributes.attr[_key22].name, "{{dummy}}");
									this.bindNode(this.attributes.getNamedItem(templateAttributes.attr[_key22].name), [], undefined, {}, templateAttributes.attr[_key22], undefined, undefined, true);
								}
							}
						}
					}
				}
				for (var _key23 in this._toRegEvnts) {
					this.addEventListener(_key23, this._toRegEvnts[_key23].listener);
					if (this.hasAttribute(_key23)) {
						this.setAttribute(_key23, this.getAttribute(_key23) + " ; " + this._toRegEvnts[_key23].attrVal);
					} else {
						this.setAttribute(_key23, this._toRegEvnts[_key23].attrVal);
					}
				}
				this._toRegEvnts = {};
				var initialUnbound = _LC.unbound;
          ...
          delete actData._bindings;
				...
				for (var i = 0; i < self.__dc.length; i++) {
					...
					if (helper.hasAttribute("lyte-keep-alive")) {
						...
						for (var j = 0; j < objKeys.length; j++) {
							var _key24 = objKeys[j];
							if (_key24 !== helper._currentComponent) {
								// Will remove from hDiv.
								helper._renderedComponent[_key24].remove();
							}
						}
					}
				}
			...
			value: function disconnectedCallback() {
				...
				for (var _key25 in this._properties) {
					this._properties[_key25] = {};
				}
			...
			value: function getArrayIndex(array, value) {
				for (var _i43 = 0; _i43 < array.length; _i43++) {
					if (array[_i43] === value) {
						return _i43;
					};
				}
			...
			value: function getTrimmedContent(content, position, node) {
				...
				for (var _i44 = 0; _i44 < position.length; _i44++) {
					for (var j = content.childNodes.length - 1; j > position[_i44]; j--) {
						content.childNodes[j].remove();
					}
					content = content.childNodes[position[_i44]];
				}
				return dummyContent;
			}
		}, {
			...
			value: function updateValue(property, path, value) {
				...
				for (var _i45 = 0; _i45 < pathVals.length - 1; _i45++) {
					context = context[pathVals[_i45]];
				}
			...
			value: function _registerComponent(a, b) {
				...
				if (this._dynamicNodes) {
					...
					if (this._templateAttributes && this._templateAttributes.attr) {
						...
						try {
							for (var _i47 = 0; _i47 < attributesT.length; _i47++) {
								if (!this._templateAttributes.attr[attributesT[_i47].name] && attributesT[_i47].name !== "tag-name" && attributesT[_i47].name !== "use-strict") {
									this._templateAttributes.attr[attributesT[_i47].name] = { "name": attributesT[_i47].name, "staticValue": attributesT[_i47].value };
								}
							}
						} catch (e) {
			...
			value: function splitTextNodes(node) {
				if (node && node.childNodes && node.childNodes.length) {
					for (var _i48 = node.childNodes.length - 1; _i48 >= 0; _i48--) {
						this.splitTextNodes(node.childNodes[_i48]);
					}
				}
				...
				if (node.nodeType === node.TEXT_NODE) {
					...
					if (nodeValue) {
						...
						var newNodeArray = [];
						for (var _i49 = 0; _i49 < mustacheValues.length; _i49++) {
							var mustacheStartIndex = nodeValue.indexOf(mustacheValues[_i49]);
							var mustacheEndIndex = mustacheStartIndex + mustacheValues[_i49].length;
						...
						if (_Lyte._rwpf && node.parentNode.nodeName === "#document-fragment") {
							for (var _i50 = 0; _i50 < newNodeArray.length; _i50++) {
								_insertBefore(node.parentNode, newNodeArray[_i50], node);
							}
							node.remove();
						} else {
			...
			get: function get() {
				var newArr = [];
				for (var _i51 = 0; _i51 < this._observedAttributes.length; _i51++) {
					newArr[_i51] = _LC.String.dasherize(this._observedAttributes[_i51]);
				}
	...
	var LyteComponent = {
		...
		"updateCustomCrmComponent": function updateCustomCrmComponent(def, customCrmComponent) {
			for (var _key26 in def) {
				if (def[_key26] && def[_key26].type === "observer") {
					customCrmComponent._observers.push(def[_key26]);
				} else if (def[_key26] && def[_key26].type === "callBack") {
					var props = def[_key26].properties;
					for (var k = 0; k < props.length; k++) {
						if (!customCrmComponent._callBacks[props[k]]) {
							customCrmComponent._callBacks[props[k]] = [];
						}
						customCrmComponent._callBacks[props[k]].push(def[_key26]);
					}
					if (def[_key26].observes) {
						customCrmComponent._observers.push(def[_key26].observes);
					}
				} else {
					customCrmComponent.component.prototype[_key26] = def[_key26];
				}
			}
			return def;
		},
		"dataFromMixin": function dataFromMixin(mixinsToBeUsed, actionsFromMixin, methodsFromMixin, newDefinition) {
			...
			for (var _i52 = 0; _i52 < mixinslen; _i52++) {
				for (var item in mixinsToBeUsed[_i52]) {
					if (item === "actions") {
						Object.assign(actionsFromMixin, mixinsToBeUsed[_i52][item]);
					} else if (item === "methods") {
						Object.assign(methodsFromMixin, mixinsToBeUsed[_i52][item]);
					} else {
						newDefinition[item] = mixinsToBeUsed[_i52][item];
					}
				}
			}
		...
		"throwEvent": function throwEvent(eventName) {
			...
			var customArgs = [];
			if (arguments.length > 1) {
				for (var _i53 = 1; _i53 < arguments.length; _i53++) {
					customArgs.push(arguments[_i53]);
				}
			}
			//wait for release
   ...
   */
		...
		"update": function update(object, property, value, fromStore, oldValue, setterScope, actualProperty, fromParent) {
			...
			if (value && typeof value !== "string" && typeof value !== "boolean" && typeof value !== "number") {
				...
				if (oldValue && (typeof oldValue === 'undefined' ? 'undefined' : _typeof(oldValue)) === "object" && oldValue._bindings) {
					...
					if (fromComponent && fromComponent.component.data === object && property.indexOf('.') === -1) {
						...
						if (object._bindings) {
							...
							for (var _i54 = 0; _i54 < oldbind.length; _i54++) {
								var _item = oldbind[_i54][property];
								if (_item) {
				...
				if (oldValue && (typeof oldValue === 'undefined' ? 'undefined' : _typeof(oldValue)) === "object" && oldValue._bindings && object._bindings) {
					...
					for (var _i56 = 0; _i56 < _objbind.length; _i56++) {
						var _item3 = _objbind[_i56];
						if (_item3[property]) {
				...
				if (object._bindings) {
					...
					for (var _i57 = 0; _i57 < _objbind2.length; _i57++) {
						var _item4 = _objbind2[_i57];
						if (_item4[property]) {
		...
		"set": function set(object, property, value, fromStore, fromParent) {
			...
			if (property instanceof Object) {
				if (_Lyte.isRecord(object) && !fromStore) {
					...
					//     }
					// }
					for (var _key27 in property) {
						var locValue = property[_key27];
						var dataType = object.$.model.fieldList[_key27];
						...
						if (dataType && (locValue !== undefined || dataType === "boolean") && dataType !== _LC.getDataType(locValue)) {
							property[_key27] = _LC.typeCast(locValue, dataType);
						}
					}
		...
		"aF": function aF() {
          ...
          continue;
								...
								for (var _key29 in item) {
									var parsedKey = parseInt(_key29);
									if (!isNaN(parsedKey) && parsedKey >= options.firstIndex) {
										...
										this.removeSelectedBindingDeep(item[_key29], _oldObject);
										if (item[_key29]._forHelpers) {
											var bindfor = item[_key29]._forHelpers.toArrayLyte();
										...
										if (array[parsedKey] && _typeof(array[parsedKey]) === "object") {
											...
											this.establishBindings({ "dummy": item[_key29] }, { "dummy": array[parsedKey] });
										}
										this.affectChanges(item[_key29]);
									}
								}
							}
						}
          ...
          continue;
								...
								for (var _key30 in _item5) {
									var _parsedKey = parseInt(_key30);
									if (!isNaN(_parsedKey) && _parsedKey >= _options.firstIndex) {
										...
										this.removeSelectedBindingDeep(_item5[_key30], _oldObject2);
										if (_item5[_key30]._forHelpers) {
											var _bindfor = _item5[_key30]._forHelpers.toArrayLyte();
										...
										if (array[_parsedKey] && _typeof(array[_parsedKey]) === "object") {
											...
											this.establishBindings({ "dummy": _item5[_key30] }, { "dummy": array[_parsedKey] });
										}
										this.affectChanges(_item5[_key30]);
									}
								}
							}
						}
          ...
          continue;
								...
								for (var _key31 in _item7) {
									var _parsedKey2 = parseInt(_key31);
									if (!isNaN(_parsedKey2) && _parsedKey2 >= _options2.firstIndex) {
										...
										this.removeSelectedBindingDeep(_item7[_key31], _oldObject3);
										if (_item7[_key31]._forHelpers) {
											var _bindfor2 = _item7[_key31]._forHelpers.toArrayLyte();
										...
										if (array[_parsedKey2] && _typeof(array[_parsedKey2]) === "object") {
											...
											this.establishBindings({ "dummy": _item7[_key31] }, { "dummy": array[_parsedKey2] });
										}
										this.affectChanges(_item7[_key31]);
									}
								}
							}
						}
				...
				case "insertAt":
					{
						...
						for (var _i65 = _index4; _i65 > array.length; _i65--) {
						...
						if (array._bindings) {
							...
							for (var _i66 = 0; _i66 < arrbind.length; _i66++) {
								var _item9 = arrbind[_i66];
          	...
          	continue;
								...
								for (var _key32 in _item9) {
									var _parsedKey3 = parseInt(_key32);
									if (!isNaN(_parsedKey3) && _parsedKey3 >= _options3.firstIndex) {
										this.removeSelectedBindingDeep(_item9[_key32], array[_parsedKey3 + _options3.secondIndex]);
										if (_item9[_key32]._forHelpers) {
											var _bindfor3 = _item9[_key32]._forHelpers.toArrayLyte();
										...
										if (array[_parsedKey3] && _typeof(array[_parsedKey3]) === "object") {
											...
											this.establishBindings({ "dummy": _item9[_key32] }, { "dummy": array[_parsedKey3] });
										}
										this.affectChanges(_item9[_key32]);
									}
								}
							}
						}
		...
		"callArrayObservers": function callArrayObservers(array, args) {
			if (array._bindings) {
				var objbind = array._bindings.toArrayLyte();
				for (var _i67 = 0; _i67 < objbind.length; _i67++) {
					var binding = objbind[_i67];
					var path = objbind[_i67]._path;
		...
		"establishUpdateBindings": function establishUpdateBindings(bindings, property, actualData) {
			...
			for (var _i68 = 0; _i68 < objbind.length; _i68++) {
				var item = objbind[_i68];
				if (item[property]) {
		...
		"establishSelectedBinding": function establishSelectedBinding(property, actualData, node) {
			...
			for (var _i69 = 0; _i69 < props.length; _i69++) {
				...
				currentProp = currentProp[props[_i69 + 1]];
				currentValue = currentValue[props[_i69 + 1]];
			}
		},
		"establishBindings": function establishBindings(properties, actualData) {
						...
						for (var j = 0; j < nodes.length; j++) {
							...
							if (helper && helper.tagName === "TEMPLATE" && helper.getAttribute("is") === "for") {
								if (helper._items) {
									...
									for (var _i70 = 0; _i70 < helper._items.length; _i70++) {
										var data = actualData[_i70];
										var _item11 = helper.getAttribute("item");
										if (data) {
											if (_typeof(helper._items[_i70]) === "object") {
												this.establishBindings(helper._items[_i70].itemProperty, _defineProperty({}, _item11, data));
											}
										}
									}
								}
			...
			for (var _i71 in properties) {
				var actData = actualData[_i71];
				if (!actData || typeof actData === "string" || typeof actData === "number" || typeof actData === "boolean") {
					...
					actData._bindings.add(properties[_i71]);
					if (_typeof(properties[_i71]) === "object") {
						this.establishBindings(properties[_i71], actData);
					}
				}
			}
		},
		"removeSelectedBindingDeep": function removeSelectedBindingDeep(binding, actualData) {
			...
			for (var _i72 in binding) {
				var actData = actualData[_i72];
				if (actData && actData._bindings) {
					//                    actData._bindings[del](binding[i]);
					//                    if(!actData._bindings.size) {
					//                        delete actData._bindings;
					//                    }
					deleteBindingCheckSize(actData, "_bindings", binding[_i72]);
				}
				if (_typeof(binding[_i72]) === "object" && actData) {
					this.removeSelectedBindingDeep(binding[_i72], actData);
				}
			}
			if (binding._forHelpers) {
				var objbind = binding._forHelpers.toArrayLyte();
				for (var _i73 = 0; _i73 < objbind.length; _i73++) {
					var fH = objbind[_i73];
					if (fH.getAttribute("is") === "for") {
						var item = fH.getAttribute("item");
						var items = fH._attributes.items;
						var itemCases = fH._items;
						for (var _i74 = 0; _i74 < itemCases.length; _i74++) {
							this.removeSelectedBindingDeep(itemCases[_i74].itemProperty, items[_i74]);
						}
					} else {
		...
		"removeAllBindings": function removeAllBindings(properties, data) {
			var del = "delete";
			for (var _key33 in properties) {
				if (data[_key33] && data[_key33]._bindings) {
					//                    data[key]._bindings[del](properties[key]);
					//                    if(!data[key]._bindings.size) {
					//                        delete data[key]._bindings;
					//                    }
					deleteBindingCheckSize(data[_key33], "_bindings", properties[_key33]);
				}
				if (data[_key33] && typeof data[_key33] !== "string") {
					_LC.removeAllBindings(properties[_key33], data[_key33]);
				}
			}
		},
		"affectChanges": function affectChanges(item, contextAlreadySwitched, oldValue, setterScope, newValue) {
			if (item._dynamicNodes) {
				for (var _i75 = 0; _i75 < item._dynamicNodes.length; _i75++) {
					item._dynamicNodes[_i75]._callee.updateNode(item._dynamicNodes[_i75], item._path);
				}
			}
			...
			if (item._helperNodes) {
				...
				for (var _s3 = 0; _s3 < itemHelperNodes.length; _s3++) {
					...
					if (itemHelperNodes[_s3].getAttribute("is") === "for" && itemHelperNodes[_s3]._items) {
						...
						for (var _i76 = 0; _i76 < itemHelperNodes[_s3]._items.length; _i76++) {
							callee.component.data[itemValue] = items[_i76];
							callee.component.data[indexValue] = _i76;
							callee._properties[itemValue] = itemHelperNodes[_s3]._items[_i76].itemProperty;
							if (itemHelperNodes[_s3]._items[_i76]._dynamicProperty[propPath]) {
								nodes = itemHelperNodes[_s3]._items[_i76]._dynamicProperty[propPath];
								for (var _i77 = 0; _i77 < nodes.length; _i77++) {
									nodes[_i77]._callee.updateNode(nodes[_i77], propPath);
								}
							}
						}
						callee.component.data[itemValue] = initialItemValue;
						...
						for (var _key34 in itemHelperNodes[_s3]._items) {
							_callee5.component.data[_itemValue3] = object[_key34];
							_callee5.component.data[_indexValue] = _key34;
							_callee5._properties[_itemValue3] = itemHelperNodes[_s3]._items[_key34].itemProperty;
							if (itemHelperNodes[_s3]._items[_key34]._dynamicProperty[propPath]) {
								nodes = itemHelperNodes[_s3]._items[_key34]._dynamicProperty[propPath];
								for (var _i78 = 0; _i78 < nodes.length; _i78++) {
									nodes[_i78]._callee.updateNode(nodes[_i78], propPath);
								}
							}
						}
						...
						if (nodes.length) {
							...
							for (var _i79 = 0; _i79 < nodes.length; _i79++) {
								nodes[_i79]._callee.updateNode(nodes[_i79], item._path);
							}
			...
			if (Array.isArray(item)) {
				for (var i = 0; i < item.length; i++) {
					for (var _key35 in item[i]) {
						this.affectChanges(item[i][_key35], true, oldValue ? oldValue[i] ? oldValue[i][_key35] : oldValue[i] : oldValue, setterScope, newValue ? newValue[i] ? newValue[i][_key35] : newValue[i] : newValue);
					}
				}
			} else {
				for (var _key36 in item) {
					this.affectChanges(item[_key36], true, oldValue ? oldValue[_key36] : oldValue, setterScope, newValue ? newValue[_key36] : newValue);
				}
			}
		},
		...
		"componentSetMethods": function componentSetMethods(arg0, arg1) {
			if (typeof arg0 === "string") {
				...
				for (var _key38 in arg0) {
					this.methods[_key38] = arg0[_key38];
				}
			}
		},
	...
	function getHelperInfo(dynamicValue, helperValue, newCompile) {
		...
		var args = getHelperArgs(helperValue.substr(1, helperValue.length - 2), newCompile);
		if (args === false) {
			return false;
		}
	...
	function getHelperArgs(str, newCompile) {
		...
		for (var _i82 = 0; _i82 < str.length; _i82++) {
			if (!stack.length && str.charAt(_i82) === ",") {
				...
				_toPush2 = getHelperArgValue(_toPush2, newCompile);
				args.push(_toPush2);
				from = _i82 + 1;
				...
				case "attr":
					{
						...
						for (var _i83 = 0; _i83 < node.attributes.length; _i83++) {
							if (node.attributes[_i83].nodeValue.indexOf("{{") !== -1) {
								...
								if (actObj && (actObj.name === "action" || actObj.name === "method") && /^(onfocus|onfocusin|onfocusout|onresize|onscroll|onclick|ondblclick|onmousedown|onmouseup|onmousemove|onmouseover|onmouseout|onchange|onselect|onsubmit|onkeydown|onkeypress|onkeyup|oncontextmenu)$/.test(node.attributes[_i83].name)) {
									attr[node.attributes[_i83].name.substr(2)] = { name: node.attributes[_i83].name.substr(2), camelCase: _LC.String.toCamelCase(node.attributes[_i83].name.substr(2)), helperInfo: actObj, newHelperInfo: getHelper(actValue, true), globalEvent: true };
									...
									if (dynNewCompile) {
										dynNewCompile.setAttribute(node.attributes[_i83].name.substr(2), componentName + " => " + actString);
									}
						...
						if (toBeRemoved.length) {
							for (var _i84 = 0; _i84 < toBeRemoved.length; _i84++) {
								...
								if (dynNewCompile) {
									dynNewCompile.removeAttribute(toBeRemoved[_i84]);
								}
							}
						}
						if (dynNewCompile) {
				...
				case "switch":
					{
						...
						if (_Lyte._ms) {
							...
							if (info[def].dynamicNodes) {
								...
								doCompile(_LC.getContentForIE(defCase, undefined, true), info[def].dynamicNodes, componentName, constr, _contentNewCompile ? _LC.getContentForIE(_contentNewCompile) : undefined);
							...
							for (var _key39 in info.cases) {
								var _contentNewCompile2 = void 0;
								if (actualTemplateNewCompile) {
									_contentNewCompile2 = _LC.getContentForIE(actualTemplateNewCompile.querySelector("[case='" + LyteComponent.cssEscape(_key39) + "']"));
								}
								content = _LC.getContentForIE(dynNContent.querySelector("[case='" + _LC.cssEscape(_key39) + "']"), undefined, true);

								doCompile(content.tagName === "TEMPLATE" ? content.content : content, info.cases[_key39].dynamicNodes, componentName, constr, _contentNewCompile2);
								if (actualTemplateNewCompile) {
									info.cases[_key39]._sta = _LC.processStatic(_contentNewCompile2);
								}
							}
							if (info[def].dynamicNodes) {
						...
						if (info.actualTemplate) {
							_depthTemp = type;
							if (!_Lyte._ie) {
								_depthTemp = document.createElement("template");
								_depthTemp.setAttribute("is", type);
								_depthTemp.innerHTML = dynN.innerHTML;
								constr.splitTextNodes(_depthTemp);
								constr._depthTemp.content.appendChild(_depthTemp);
							}
							if (dynamicNodes[j - 1] && dynamicNodes[j - 1].position.toString() === dynamicNodes[j].position.toString()) {
	...
	function getArgString(name, array) {
		...
		for (var _i86 = 0; _i86 < array.length; _i86++) {
			if (array[_i86] && _typeof(array[_i86]) === "object") {
				array[_i86] = getArgString(array[_i86].value.name, array[_i86].value.args);
			}
		}
		if (name) {
		...
		value: function value(arg0, arg1) {
			...
			if (typeof arg0 === "string") {
				...
				for (var _key40 in arg0) {
					this._initProperties[_key40] = arg0[_key40];
				}
			}
		}
	});
		...
		value: function value(arg0, arg1) {
			...
			if (typeof arg0 === "string") {
				...
				for (var _key41 in arg0) {
					this._initMethods[_key41] = arg0[_key41];
				}
			}
		}
	});
	...
	$: {
		...
		removeSelfCircularReference: function removeSelfCircularReference(modelName, obj, expose, type, partialObj) {
			...
			for (var key in obj) {
				...
				if (expose == true && obj[key] && _typeof(obj[key]) == "object" && field && field.type != "relation") {
					obj[key] = Lyte.deepCopyObject(obj[key]);
					continue;
				}
		...
		removeFromStore: function removeFromStore(model, keys, fromStore, ignorePartial) {
			var data = model.data;
			if (data.length == 0) {
				return;
			}
		...
		toDemolishRelation: function toDemolishRelation(model, index, ignorePartial) {
		...
		rollBackDelete: function rollBackDelete(model, pkVal, index) {
			var arr = [];
			if (!pkVal) {
...
store.adapter = {
	$: {
		...
		getFailure: function getFailure(modelName, type, key, urlObj, xhr, resolve, reject, opts, content, code) {
		...
		handleRequest: function handleRequest(urlObj, modelName, data, type, changedData, customData, partialObj, key, actionName) {
			...
			return new Promise(function (resolve, reject) {
				...
				if (processRequest) {
					sendXHR = false;
					var returnPromise = self.callGeneric(type, modelName, urlObj.data, data, customData, urlObj ? urlObj.qP : undefined, key, urlObj.url, actionName),
					    response;
					if (returnPromise instanceof Promise) {
						batchPro = true;
						returnPromise.then(function (resp) {
		...
		handleFailure: function handleFailure(modelName, type, xhr, data, urlObj, resolve, respObj, partialObj, reject, key, customData, code) {
			if (xhr) {
				var resp;
				var scope = store.$.cbScp(modelName, "parseResponse"),
				    args,
