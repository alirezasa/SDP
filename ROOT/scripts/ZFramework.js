var ZAppUtil = (function (ZAppUtil) { // eslint-disable-line no-unused-vars, no-shadow
  ...
  function replaceUrlData(resolvedURL, envData, configData) {
    var resolvedUrl = resolvedURL;
    if ( typeof (resolvedURL) === 'string') {
      if ( configData && Array.isArray(configData)) {
        configData.forEach(function (configdata) {
          for (var key in configdata) {
            if (configdata.hasOwnProperty(key)) {
              resolvedUrl = resolvedURL.replace(new RegExp('{{\\s*' + key + '\\s*}}', 'g'), configdata[key]);
            }
          }
        });
      }
      if ( envData && typeof (envData) === 'object' ) {
        for (var key in envData) {
          if (envData.hasOwnProperty(key)) {
            resolvedUrl = resolvedURL.replace(new RegExp('{{\\s*' + key + '\\s*}}', 'g'), envData[key]);
          }
        }
      }
      return resolvedUrl;
  ...
  ZAppUtil.replaceUrlData = replaceUrlData;
  return ZAppUtil;
})(window.ZAppUtil || {});
...
ZWidgetRuntime.prototype.getURL = function() {
    if(this.__WIDGET__.other_dc_urls){
        var urlObj = this.__WIDGET__.other_dc_urls;
        return urlObj[ZApp.GetDCType()] ? urlObj[ZApp.GetDCType()] : this.__WIDGET__.url;
    }
...
ZWidgetRuntime.prototype.getWidgetMeta = function () {
    return this.__WIDGET__ && this.__WIDGET__.widgetmeta;
}
  ...
  function WidgetLoadHandler(widgetInstance) {
    ...
    var PluginContext = {
      ...
      data: {
        ...
        widgetmeta: widgetInstance.getWidgetMeta(),
        uniqueID: widgetInstance.getUniqueID(),
  ...
  try {
    ...
    var
      ...
      serverConfig = {
        ...
        zappsHost: {
          host: '{{zappid}}.zappsusercontent.' + TLD,
          port: '',
    ...
    function setTopLevelDomain() {
      if(dcType == null && TLD == null) {
        TLD = window.location.origin.split(".").pop().toLowerCase();
        dcType = "US";
        if (TLD === "cn") {
          dcType = "CN";
          TLD = 'com.cn';
        ...
        if (TLD.contains("cn")) {
          dcType = "CN";
        } else if (TLD.contains("au")) {
          dcType = "AU";
        } else if(TLD.contains("eu")) {
          dcType = "EU";
        } else if(TLD.contains("in")) {
          dcType = "IN";
        } else if(TLD.contains("jp")) {
          dcType = "JP";
        }

      } 
      serverConfig.zappsHost.host = '{{zappid}}.zappsusercontent.' + TLD;
    }
    ...
    function RenderWidgets(locationID, modelData, envData ) {
      ...
      for ( var i = 0; i < Widgets.length; i++) {
        ...
        try {
          ...
          RenderWidget(locationID, widgetInstance, envData);
        } catch (e) {
    ...
    function RenderWidget(renderLocation, widgetInstance, envData) {

      try {
        var uniqueID = generateSecretKey();
        widgetInstance._uniqueID = uniqueID;
        widgetInstance._resolvedURL = getResolvedAppURL(widgetInstance, envData);
    ...
    function changeDevHost(devHost) {
      if(devHost) {
        serverConfig.devHost = devHost;
      }
    }
    ...
    function getResolvedAppURL(widgetConfig, envData) {
      ...
      var frameorigin = isDevMode() ? serverConfig.devHost.frameOrigin : serverConfig.zappsHost.frameOrigin;
      if(frameorigin != undefined) {
        QPString = QPString + "&" + frameorigin;
      }
      ...
      var configData = ZApp.GetExtensionConfig(extID);
      if(envData || configData) {
        resolvedURL = ZAppUtil.replaceUrlData(resolvedURL, envData, configData);
      }

      return resolvedURL;
    }
    function getConstructedURL(extensionData) {
      // config changes for zuid starts
      if(extensionData.zappid && extensionData.zappid.indexOf('-') > -1) {
        serverConfig.zappsHost.path = '/appfiles/v2/{{zappid}}/{{version}}/{{hashkey}}';
      } else {
        serverConfig.zappsHost.path = '/appfiles/{{zappid}}/{{version}}/{{hashkey}}';
      }
      if(extensionData.revision){
        serverConfig.zappsHost.path += '/{{revision}}';
      }
      // config changes for zuid ends
      var urlConfig = serverConfig.zappsHost;
      if (isDevMode()) {
        urlConfig = serverConfig.devHost;
      }
      var extndata = extensionData.extensiondata;
      if(extndata != undefined && extndata.appdctype != undefined && extndata.appdctype === 3 && urlConfig.host.search("zappsusercontent") != -1) {
        urlConfig.replace("zappsusercontent", "sigmausercontent");
      }
      var fullURL;
    ...
    // Scheduler
    // function CleanRuntimeScheduler() {
    //   var keys = Object.keys(WidgetRuntimes);
    ...
    //  setTimeout( CleanRuntimeScheduler, CLEAN_RUNTIME_TIMEOUT);
    // }
    ...
    Framework.toggleDevMode = toggleDevMode;
    Framework.changeDevHost = changeDevHost;
    ...
    Framework.GetDCType = GetDCType;
    Framework.GetTopLevelDomain = GetTopLevelDomain;
